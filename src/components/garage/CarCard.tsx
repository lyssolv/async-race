...
export default function CarCard({ car, selected, onSelect, onDelete, onStart, onStop }: Props) {
  return (
    <article className={`car-row${selected ? ' selected' : ''}`} aria-label={`${car.name} lane`}>
      <aside className="car-tile">
        ...
        <div className="tile-drive">
          <Button size="sm" color="yellow" onClick={onStart} aria-label="Start engine">
            A
          </Button>
          <Button size="sm" color="white" onClick={onStop} aria-label="Stop engine">
            B
          </Button>
        </div>
